...
exports.submitSurvey = exports.getSurveyToFill = exports.getPublishedSurveys = void 0;
...
exports.getPublishedSurveys = getPublishedSurveys;
...
const getSurveyToFill = async (req, res) => {
    try {
        ...
        const existingSubmission = await survey_submission_1.default.findOne({ surveyId: survey._id, userId });
...
const submitSurvey = async (req, res) => {
    ...
    try {
        const survey = await survey_1.default.findById(surveyId).session(session);
        if (!survey || survey.status !== 'published') {
            await session.abortTransaction();
            return res.status(404).json({ message: 'Survey not found or is no longer active.' });
        }
        const existingSubmission = await survey_submission_1.default.findOne({ surveyId, userId }).session(session);
        if (existingSubmission) {
            await session.abortTransaction();
        ...
        const submission = new survey_submission_1.default({
            surveyId,
            userId,
        ...
        if (!isFraudulent && survey.pointsAwarded > 0) {
            ...
            if (user) {
                ...
                const transaction = new point_transaction_1.default({
                    userId: userId,
                    type: 'credit',
...
//# sourceMappingURL=userSurvey.controller.js.map