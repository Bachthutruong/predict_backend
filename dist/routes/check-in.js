...
router.get('/question/public', auth_1.authenticate, async (req, res) => {
    try {
        // Fetch one random active question
        const question = await question_1.default.aggregate([
            { $match: { status: 'active' } },
            { $sample: { size: 1 } }
        ]);
        ...
        await question_1.default.findByIdAndUpdate(question[0]._id, {
            $inc: { displayCount: 1 }
        });
...
router.get('/question', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        // Check if already checked in today
        const today = new Date();
        ...
        let question = await question_1.default.findOne({
            status: 'active',
            isPriority: true,
        ...
        if (!question) {
            question = await question_1.default.findOne({
                status: 'active',
                _id: { $nin: user.answeredQuestionIds }
        ...
        if (!question) {
            ...
            question = await question_1.default.findOne({ status: 'active' })
                .sort({ isPriority: -1, displayCount: 1 });
        }
        if (!question) {
            return res.status(404).json({
        ...
        // Update display count for the question
        await question_1.default.findByIdAndUpdate(question._id, {
            $inc: { displayCount: 1 }
        });
        res.json({
            success: true,
...
router.post('/submit', auth_1.authenticate, async (req, res) => {
    try {
        ...
        await question_1.default.findByIdAndUpdate(questionId, {
            $inc: { displayCount: 1 }
        });
        ...
        let displayConsecutiveCheckIns = undefined; // For UI display only
...
router.post('/reset-skips', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        const today = new Date();
        today.setHours(0, 0, 0, 0);
