...
function isPredictionActive(p: any): boolean {
...
router.get('/', async (req, res) => {
  try {
    ...
    const winnerCounts = await UserPrediction.aggregate([
...
router.get('/:id', optionalAuthenticate, checkPredictionViewAccess as any, async (req: any, res) => {
  try {
    ...
    const winnerCount = await UserPrediction.countDocuments({ predictionId: req.params.id, isCorrect: true });
    ...
    if (prediction.status === 'active' && predictionObj.endDate && new Date(predictionObj.endDate) < now) {
      await Prediction.findByIdAndUpdate(req.params.id, { status: 'finished' });
      predictionObj.status = 'finished';
      prediction.status = 'finished' as any;
    }
    ...
    const transformedUserPredictions = userPredictions.map((up: any) => {
      const item: any = { ...up, id: up._id.toString(), user: up.userId };
    ...
    let userPredictionsForUser: any[] = [];
    if (userId) {
      userAttemptCount = await UserPrediction.countDocuments({ userId, predictionId: req.params.id });
      userPredictionsForUser = await UserPrediction.find({ userId, predictionId: req.params.id })
        .sort({ createdAt: -1 })
        .lean();
    }
    ...
    res.json({
      ...
      data: {
        ...
        totalPages,
        userAttemptCount,
        userPredictionsForUser: userPredictionsForUser.map((up: any) => {
          const item: any = { ...up, id: up._id.toString() };
          if (!canViewCorrectness) delete item.isCorrect;
          return item;
        })
      }
    });
...
router.post('/:id/submit', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const predObj = prediction.toObject() as any;
    const now = new Date();
    ...
    const userAttemptCount = await UserPrediction.countDocuments({ userId, predictionId });
    ...
    if (isCorrect) {
      const winnerCount = await UserPrediction.countDocuments({ predictionId, isCorrect: true });
        ...
        try {
          ...
          if (rewards.length > 0) {
            ...
            for (const r of rewards) {
              if (r.type === 'points' && r.pointsAmount) pointsToAward += r.pointsAmount;
            }
          }
          if (pointsToAward > 0) {
            await User.findByIdAndUpdate(userId, { $inc: { points: pointsToAward } }, { session });
          ...
          for (const r of rewards) {
            if (r.type === 'product' && r.productId && r.productQuantity) {
              const product = await Product.findById(r.productId).session(session);
              if (product && product.stock >= r.productQuantity) {
                const prevStock = product.stock;
                await Product.findByIdAndUpdate(r.productId, { $inc: { stock: -r.productQuantity } }, { session });
          ...
          const winnerIds: any[] = [...(prediction.winnerIds || [])];
          const alreadyWinner = winnerIds.some((id: any) => (id?.toString?.() || id) === userId);
          if (!alreadyWinner) {
