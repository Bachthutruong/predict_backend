...
export const createVotingTestData = async () => {
  try {
    ...
    const existingCampaigns = await VotingCampaign.find({ 
    ...
    const campaigns = [
      ...
      },
      {
        ...
        pointsPerVote: 15,
        maxVotesPerUser: 5,
        votingFrequency: 'daily' as const,
    ...
    const createdCampaigns = await VotingCampaign.insertMany(campaigns);
    ...
    const createdEntries = await VoteEntry.insertMany(entries);
